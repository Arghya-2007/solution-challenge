...
class ImpersonatedSigner {
  ...
  constructor(sourceClient: any, targetPrincipal: string) {
...
async function test() {
  ...
  const customSigner = new ImpersonatedSigner(sourceClient, 'csv-upload-signer@project-68c64ea9-e771-4bee-9f1.iam.gserviceaccount.com');

  const storage = new Storage({
    projectId: 'equilens-e21e9',
    authClient: customSigner as any,
  });
